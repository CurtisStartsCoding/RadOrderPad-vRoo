-- Migration: Revert prompt template changes while keeping specialty_configurations table
-- Date: 2025-04-15

-- Connect to the radorder_main database
\c radorder_main;

-- Backup the current prompt template (just in case)
...
-- Revert the prompt template to its original state
-- Note: This assumes we're reverting to the original template that used 'diagnosisCodes' and 'procedureCodes'
