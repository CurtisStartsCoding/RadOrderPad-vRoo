-- Migration: Update prompt template to use specialty name instead of null for feedback
-- Date: 2025-04-15

-- Connect to the radorder_main database
\c radorder_main;

-- Update the prompt template to use specialty name instead of null for feedback
...
```json
{
  ...
  "feedback": "string (use ''{{SPECIALTY}}'' if validationStatus is ''valid'', otherwise brief educational note - aim for ~{{TARGET_WORD_COUNT}} words for the {{SPECIALTY}} specialty)"
}
```
...
- The `feedback` field MUST be "{{SPECIALTY}}" if `validationStatus` is ''valid''.
