...
import { getActivePromptTemplate, generateDatabaseContextWithRedis, constructPrompt } from '../../utils/database';
 ...
 */
export async function runValidation(text, context = {}, options = {}) {
    try {
        ...
        const prompt = constructPrompt(promptTemplate.content_template, sanitizedText, databaseContext, 33, // Hard-coded to 33 words
        ...
        if (!options.testMode) {
            await logValidationAttempt(text, validationResult, llmResponse, context.orderId, context.userId || 1 // Default to user ID 1 if not provided
            );
