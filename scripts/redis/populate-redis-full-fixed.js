/**
 * Script to populate Redis with ALL data from PostgreSQL - Fixed Version
 * 
 * This script uses batch operations and pipelines for faster loading.
 * It's also resilient to database schema differences.
 * 
 * IMPORTANT: This version uses the correct key formats that match the service files.
 */
...
const redisClient = new Redis({
  ...
  tls: redisTls,
  connectionName: 'redis-populate-fixed',
  ...
  try {
    ...
    try {
      ...
      if (cptColumns.length > 0) {
        ...
        // Store CPT codes in Redis with FIXED key format
    ...
    try {
      ...
      if (icd10Columns.length > 0) {
        ...
        // Store ICD-10 codes in Redis with FIXED key format
    ...
    try {
      ...
      if (mappingColumns.length > 0) {
        ...
        // Store mappings in Redis with FIXED key format
...
async function createSearchIndexes() {
  try {
    ...
    // Create ICD-10 index with FIXED prefix
    ...
    // Create CPT index with FIXED prefix
    await redisClient.call(
