...
const claudeApiKey = process.env.ANTHROPIC_API_KEY; // Changed from CLAUDE_API_KEY to ANTHROPIC_API_KEY
const openaiApiKey = process.env.OPENAI_API_KEY;

if (!claudeApiKey) {
  console.warn('Warning: ANTHROPIC_API_KEY is not set in the environment variables. Claude tests will be skipped.');
}

if (!openaiApiKey) {
  console.warn('Warning: OPENAI_API_KEY is not set in the environment variables. GPT tests will be skipped.');
}
...
async function runTest(llmConfig, prompt, testCase, oldFormat) {
  try {
    ...
    if (llmConfig.name === 'Claude' && !process.env.ANTHROPIC_API_KEY) {
      console.log('Skipping Claude test because ANTHROPIC_API_KEY is not set');
      return {
        skipped: true,
    ...
    if (llmConfig.name === 'GPT' && !process.env.OPENAI_API_KEY) {
      console.log('Skipping GPT test because OPENAI_API_KEY is not set');
      return {
        skipped: true,
    ...
    const fileName = `${llmConfig.name}_${oldFormat ? 'old' : 'new'}_prompt_case${testCase.id}_error.txt`;
    fs.writeFileSync(path.join(resultsDir, fileName), error.toString());
    console.log(`Error saved to ${path.resolve(path.join(resultsDir, fileName))}`);
