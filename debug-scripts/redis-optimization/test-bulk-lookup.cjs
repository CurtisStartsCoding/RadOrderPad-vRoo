 ...
 * 
 * This script tests the bulk lookup functionality using Lua scripts
 * with the same Redis connection approach as the working test-redis-cache-v2.cjs
 */
 ...
 */
async function loadAndCacheScripts() {
  try {
    const scriptSha = await redisClient.script('LOAD', BULK_LOOKUP_SCRIPT);
    console.log(`Loaded bulk lookup script with SHA: ${scriptSha}`);
    return scriptSha;
  } catch (error) {
 ...
 */
async function testIndividualLookups() {
...
/**
 * Test bulk lookups using Lua script
 */
async function testBulkLookups(scriptSha) {
  ...
  const { result: bulkCptResults, duration: bulkCptDuration } = await measurePerformance(async () => {
    const [results, hits, misses] = await redisClient.evalsha(
      ...
      'cpt:code:', // First ARGV is the prefix
      ...TEST_CPT_CODES // Rest of ARGV are the codes
    );
    
    console.log(`Bulk CPT lookup: ${hits} cache hits, ${misses} cache misses`);
  ...
  const { result: bulkIcd10Results, duration: bulkIcd10Duration } = await measurePerformance(async () => {
    const [results, hits, misses] = await redisClient.evalsha(
      scriptSha,
      0, // No KEYS, all in ARGV
      'icd10:code:', // First ARGV is the prefix
      ...TEST_ICD10_CODES // Rest of ARGV are the codes
    );
    
    console.log(`Bulk ICD-10 lookup: ${hits} cache hits, ${misses} cache misses`);
...
function compareResults(individual, bulk) {
  ...
  // Note about performance with small datasets
  if (cptImprovement < 0 || icd10Improvement < 0) {
    console.log('\nNote: Bulk operations may be slower for small datasets due to Lua script overhead.');
    console.log('The performance benefit typically becomes apparent with larger datasets (50+ items).');
    console.log('This test focuses on functionality rather than performance optimization.');
  }
...
async function runTest() {
  ...
  try {
    ...
    // Load Lua scripts
    const scriptSha = await loadAndCacheScripts();
    
    // Setup test data
    await setupTestData();
    
    // Run individual lookups
    const individualResults = await testIndividualLookups();
    
    // Run bulk lookups
    const bulkResults = await testBulkLookups(scriptSha);
    
    // Compare results
    const comparison = compareResults(individualResults, bulkResults);
    
    // Determine test outcome - only check that results match, not performance
    // For small datasets, the Lua script overhead might make bulk operations slower
    // The performance benefit typically shows up with larger datasets
