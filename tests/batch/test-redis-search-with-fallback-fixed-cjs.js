...
// Import the Redis context generator and Redis connection functions
const redisContextGenerator = require('../../dist/utils/database/redis-context-generator-fix');
...
async function testRedisSearchSuccess() {
  ...
  try {
    ...
    const context = await redisContextGenerator.generateDatabaseContextWithRedis(keywords);
...
runTests().catch(error => {
  ...
  // Ensure all connections are closed even if there's an error
  redisConfig.closeRedisConnection().catch(closeError => {
    console.error('Error closing Redis connection:', closeError);
  }).finally(() => {
    process.exit(1);
  });
