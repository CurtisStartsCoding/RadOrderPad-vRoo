 ...
 */
export async function getOrganizationResilient(orgId: number): Promise<{
  organization: OrganizationResponse;
  locations: LocationResponse[];
  users: UserResponse[];
} | null> {
  enhancedLogger.debug('Getting organization details (resilient version)', { orgId });
  ...
  try {
    ...
    logSchemaCompatibilityError(error, { 
      orgId,
      service: 'getOrganizationResilient'
    });
