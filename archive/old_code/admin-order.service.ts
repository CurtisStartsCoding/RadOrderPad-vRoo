...
class AdminOrderService {
   ...
   */
  async handlePasteSummary(orderId: number, pastedText: string, userId: number): Promise<any> {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO patient_clinical_records
         ...
         VALUES ($1, $2, $3, $4, $5)`,
        [order.patient_id, orderId, 'emr_summary_paste', pastedText, userId]
      );
   ...
   * @returns Promise with result
   */
  async handlePasteSupplemental(orderId: number, pastedText: string, userId: number): Promise<any> {
    try {
      // 1. Verify order exists and has status 'pending_admin'
      const orderResult = await queryPhiDb(
        `SELECT o.id, o.status, o.patient_id 
         FROM orders o
         WHERE o.id = $1`,
        [orderId]
      );
      ...
      await queryPhiDb(
        `INSERT INTO patient_clinical_records
         (patient_id, order_id, record_type, content, added_by_user_id)
         VALUES ($1, $2, $3, $4, $5)`,
        [order.patient_id, orderId, 'supplemental_docs_paste', pastedText, userId]
      );
   ...
   * @returns Promise with result
   */
  async updatePatientInfo(orderId: number, patientData: any, userId: number): Promise<any> {
    try {
      // 1. Verify order exists and has status 'pending_admin'
      const orderResult = await queryPhiDb(
        `SELECT o.id, o.status, o.patient_id 
      ...
      const result = await queryPhiDb(updateQuery, [...updateValues, order.patient_id]);
   ...
   * @returns Promise with result
   */
  async updateInsuranceInfo(orderId: number, insuranceData: any, userId: number): Promise<any> {
    try {
      // 1. Verify order exists and has status 'pending_admin'
      const orderResult = await queryPhiDb(
        `SELECT o.id, o.status, o.patient_id 
      ...
      const insuranceResult = await queryPhiDb(
        ...
        [order.patient_id, insuranceData.isPrimary === false ? false : true]
      );
   ...
   * @returns Promise with result
   */
  async sendToRadiology(orderId: number, userId: number): Promise<any> {
    // Get a client for transaction
    const client = await getPhiDbClient();
    ...
    try {
      ...
      if (insuranceResult.rows.length === 0) {
        ...
        if (!insurance.insurer_name) missingPatientFields.push('insurance provider name');
        if (!insurance.policy_number) missingPatientFields.push('insurance policy number');
      }
   ...
   * @returns Parsed data
   */
  private parseEmrSummary(text: string): any {
    // This is a simple implementation that could be enhanced with more sophisticated NLP
