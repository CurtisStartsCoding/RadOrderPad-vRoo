...
import { generalNotifications } from '../../notification/services';
 ...
 */
export const verifyWebhookSignature = (payload: any, signature: string): Stripe.Event => {
 ...
 */
export const handleCheckoutSessionCompleted = async (event: Stripe.Event): Promise<void> => {
 ...
 */
export const handleInvoicePaymentSucceeded = async (event: Stripe.Event): Promise<void> => {
  ...
  try {
    ...
    const orgResult = await client.query(
      `SELECT id, type, status, subscription_tier 
       FROM organizations 
       WHERE billing_id = $1`,
      [customerId]
    );
      ...
      if (adminEmails.length > 0) {
        ...
        for (const email of adminEmails) {
          await generalNotifications.sendNotificationEmail(
            email,
            'Account Reactivated',
            `Your organization (ID: ${orgId}) has been reactivated after successful payment.`
          );
        }
      }
    }
 ...
 */
export const handleInvoicePaymentFailed = async (event: Stripe.Event): Promise<void> => {
  const invoice = event.data.object as Stripe.Invoice;
  const customerId = invoice.customer as string;
      ...
      if (adminEmails.length > 0) {
        ...
        for (const email of adminEmails) {
          await generalNotifications.sendNotificationEmail(
            ...
            `Your organization ${orgName} (ID: ${orgId}) has been suspended due to payment failure.`
          );
        }
      }
    } else {
        ...
        if (adminEmails.length > 0) {
          ...
          for (const email of adminEmails) {
            await generalNotifications.sendNotificationEmail(
              ...
              `Payment failed for invoice ${invoice.id} for organization ${orgName} (ID: ${orgId}). This is attempt ${invoice.attempt_count || 1}.`
            );
          }
        }
      }
...
export const handleSubscriptionUpdated = async (event: Stripe.Event): Promise<void> => {
  ...
  try {
    ...
    // 1. Find the organization by Stripe customer ID
    const orgResult = await client.query(
      `SELECT id, type, subscription_tier 
       FROM organizations 
       WHERE billing_id = $1`,
      [customerId]
    );
 ...
 */
export const handleSubscriptionDeleted = async (event: Stripe.Event): Promise<void> => {
  const subscription = event.data.object as Stripe.Subscription;
  const customerId = subscription.customer as string;
    ...
    if (adminEmails.length > 0) {
      ...
      for (const email of adminEmails) {
        await generalNotifications.sendNotificationEmail(
          email,
          'Subscription Canceled',
          `The subscription for organization ${orgName} (ID: ${orgId}) has been canceled.`
        );
      }
    }
